...
## Important GitHub layout
The GitHub Actions workflow must live at repository root under `.github/workflows/bridge.yml`.
The bridge writes `signals.jsonl`, `outbox.jsonl`, and `bridge_state.json` at repository root so the Windows executor and GitHub Actions use the same files.
`signals.jsonl` may be absent on first startup; the executor treats that as an empty queue only after validating repository access.
