...
def is_transient_error(e: Exception) -> bool:
    """True for errors that say nothing about the exchange-side state (rate
    limit, network hiccup, 5xx). Callers MUST NOT conclude "position closed /
    order missing" from one of these - that exact confusion wiped 11 live
    positions out of the executor's state on 2026-09-21 (TooManyRequests was
    read as "position not open")."""
    code = str(getattr(e, "code", "") or "")
    status = getattr(e, "http_status", None)
    if code in ("NetworkError", "TooManyRequests", "Throttled", "RateLimit"):
        return True
    if status is not None and (status == 429 or status >= 500):
        return True
    if code.startswith("HTTP5") or code == "HTTP429":
        return True
    return False


_WAIT_RE = re.compile(r"(\d+)\s*(ثانیه|دقیقه|ساعت|second|sec|minute|min|hour)", re.IGNORECASE)


def parse_wait_seconds(message: str, default: float = 60.0) -> float:
    """Nobitex rate-limit text: 'لطفا 2 دقیقه صبر نمایید.' -> 120."""
    m = _WAIT_RE.search(message or "")
    if not m:
        return default
    n = float(m.group(1))
    unit = m.group(2).lower()
    if unit in ("دقیقه", "minute", "min"):
        return n * 60
    if unit in ("ساعت", "hour"):
        return n * 3600
    return n
...
class NobitexClient:
    def __init__(self, config: NobitexConfig):
        ...
        # Client-side pacing. Without it, repair/verify loops fired dozens of
        # calls per second and Nobitex answered TooManyRequests ("wait 6
        # minutes"), which then cascaded into wrong state decisions.
        self._pace_lock = threading.Lock()
        self._last_request_at = 0.0
        self._blocked_until = 0.0
        self.min_interval = 0.30
        self.max_cooldown_wait = 180.0
    ...
    def _pace(self) -> None:
        """Wait out any active rate-limit cooldown, then keep a minimum gap
        between requests."""
        with self._pace_lock:
            now = time.time()
            if self._blocked_until > now:
                wait = min(self._blocked_until - now, self.max_cooldown_wait)
                log.warning("Nobitex rate-limit cooldown: waiting %.0fs before next request", wait)
                time.sleep(wait)
    ...
    def _note_rate_limit(self, message: str) -> None:
        wait = parse_wait_seconds(message, 60.0)
        self._blocked_until = max(self._blocked_until, time.time() + wait + 1.0)
    ...
    def _request(self, method: str, path: str, *, query: Optional[dict] = None,
        ...
        for attempt in range(attempts):
            self._pace()
            # Sign per attempt so the timestamp is always fresh (pacing may sleep).
            ...
            try:
                ...
                if resp.status_code == 429:
                    self._note_rate_limit(resp.text[:300])
                    raise NobitexAPIError("TooManyRequests", resp.text[:300] or "HTTP 429", data, 429)
                ...
                # a short retry is safe for network errors - but NOT after a
                # rate-limit answer (retrying only prolongs the block).
                if isinstance(e, NobitexAPIError) and e.code == "TooManyRequests":
                    raise
    ...
    def get_last_trade_price(self, market: str) -> Decimal:
        ...
        url = self.cfg.public_base_url.rstrip("/") + f"/v3/orderbook/{market.upper()}"
