...
HEARTBEAT_STALE_SECONDS = int(os.environ.get("HEARTBEAT_STALE_SECONDS", "240"))
...
PUSH_MIN_INTERVAL_SECONDS = float(os.environ.get("PUSH_MIN_INTERVAL_SECONDS", "1"))
PULL_MIN_INTERVAL_SECONDS = float(os.environ.get("PULL_MIN_INTERVAL_SECONDS", "3"))
...
BOT_COMMANDS = [
    ...
    {"command": "history", "description": "تاریخچه‌ی معاملات بسته‌شده"},
    {"command": "pnl", "description": "خلاصه‌ی سود/زیان آخرین معاملات"},
    ...
    {"command": "leverage", "description": "تنظیم اهرم ترجیحی"},
    {"command": "ack", "description": "بررسی‌شده علامت‌گذاری کن (توقف پیگیری خودکار)"},
    {"command": "fixprotection", "description": "تلاش مجدد برای تکمیل محافظت یک معامله"},
    {"command": "confirmclose", "description": "تأیید بستن معامله طبق رویداد کانال"},
    {"command": "dismissclose", "description": "رد درخواست بستن معامله طبق رویداد کانال"},
    {"command": "recover", "description": "بازیابی پوزیشن‌های بی‌صاحب (یا یک شناسه‌ی خاص)"},
    {"command": "ack_orphan", "description": "بررسی‌شده علامت‌گذاری کن (پوزیشن بی‌صاحب نامشخص)"},
    {"command": "closeorphan", "description": "بستن یک پوزیشن بی‌صاحب با شناسه‌ی آن"},
...
def send_message(text: str, inline_keyboard=None, reply_keyboard=None, reply_to_message_id=None):
...
def reply_keyboard():
    return [
        ...
        ["📈 اهرم", "🧪 تست API", "🔄 همگام‌سازی"],
        ["🛡️ بررسی محافظت", "🧾 آخرین لاگ‌ها", "📜 تاریخچه"],
        ["📊 خلاصه سود/زیان"],
        ["❌ بستن یک معامله", "🚨 بستن همه معاملات"],
        ["📋 منوی کامل"],
    ]
...
def inline_main_menu():
    return [
        ...
        [{"text": "📜 تاریخچه معاملات", "callback_data": "cmd:/history"}, {"text": "📊 خلاصه سود/زیان", "callback_data": "cmd:/pnl"}],
        [{"text": "📌 ریسک", "callback_data": "risk_menu"}, {"text": "💵 مارجین", "callback_data": "collateral_menu"}],
        [{"text": "🔢 سقف معاملات", "callback_data": "maxtrades_menu"}, {"text": "📈 اهرم", "callback_data": "leverage_menu"}],
...
def handle_message(update: dict) -> bool:
    ...
    keyboard_commands = {
        ...
        "🛡️ بررسی محافظت": "/protection", "🧾 آخرین لاگ‌ها": "/logs", "📜 تاریخچه": "/history", "📊 خلاصه سود/زیان": "/pnl",
    }
    ...
    if text == "❌ بستن یک معامله":
        ...
        send_message(
            "📈 لیست معاملات باز به‌زودی در همین چت می‌رسد — زیر آن پیام، برای هر معامله یک دکمه‌ی "
            "بستن مخصوص همان معامله می‌بینید؛ کافی است روی همان بزنید.",
    ...
    if text == "📌 ریسک":
        send_message(
            "🎯 سقف ریسک هر معامله\n\nسقف بالا وجود ندارد؛ برای هر عدد دلخواه (مثلاً هر چقدر حساب "
            "بزرگ‌تر شد) کافی است خودتان تایپ کنید:\n/risk 25",
            inline_keyboard=[
    ...
    if text == "💵 مارجین":
        send_message(
            "💵 سقف وجه تضمین هر معامله\n\nسقف بالا وجود ندارد؛ برای هر عدد دلخواه کافی است خودتان "
            "تایپ کنید:\n/collateral 25",
            inline_keyboard=[
    ...
    if text == "🔢 سقف معاملات":
        send_message(
            "🔢 حداکثر تعداد پوزیشن همزمان\n\nبرای عدد دلخواه (مثلاً 3) کافی است خودتان تایپ کنید:\n/maxtrades 3",
            inline_keyboard=[
                [{"text": "3", "callback_data": "cmd:/maxtrades 3"}, {"text": "5", "callback_data": "cmd:/maxtrades 5"}],
                [{"text": "10", "callback_data": "cmd:/maxtrades 10"}, {"text": "15", "callback_data": "cmd:/maxtrades 15"}],
    ...
    if text == "📈 اهرم":
        send_message(
            "📈 اهرم ترجیحی\n\nهمین عدد مستقیماً برای هر معامله به نوبیتکس درخواست می‌شود؛ سقف پایین‌تری بر "
            "اساس نوع بازار روی آن اعمال نمی‌کنیم. اگر نوبیتکس برای بازاری خاص واقعاً کمتر از این اجازه "
            "بدهد، همان لحظه در پیام تأیید معامله نشان داده می‌شود.",
            inline_keyboard=[
                [{"text": "3x", "callback_data": "cmd:/leverage 3"}, {"text": "5x", "callback_data": "cmd:/leverage 5"}],
                [{"text": "10x", "callback_data": "cmd:/leverage 10"}, {"text": "20x", "callback_data": "cmd:/leverage 20"}],
                [{"text": "↩️ منو", "callback_data": "menu"}],
            ],
        )
        return True
    ...
    for idx in range(sent, len(lines)):
        ...
        try:
            msg_id = send_message(
                ...
                reply_to_message_id=(thread_roots.get(key) if key else None),
            )
            if key and msg_id and key not in thread_roots:
