    ...
    try:
        ...
        if cmd=="/history":
            ...
            for h in recent:
                ...
                pnl_raw = h.get("realized_usdt_estimate")
                pnl_line = ""
                if pnl_raw is not None:
                    try:
                        pv = D(pnl_raw)
                        pnl_line = f"\nسود/زیان تخمینی: {'🟢+' if pv>=0 else '🔴'}{pv:.4f} USDT"
                    except (InvalidOperation, TypeError):
                        pass
                lines.append(
        ...
        if cmd=="/pnl":
            ...
            if not sample:
                return "📊 هنوز هیچ معامله‌ای در تاریخچه ثبت نشده."
            total = Decimal("0"); wins = 0; losses = 0; flat = 0; counted = 0
            for h in sample:
                raw = h.get("realized_usdt_estimate")
                if raw is None:
                    continue
                try:
                    v = D(raw)
            ...
            lines = [
                ...
                "",
                "⚠️ این اعداد بر پایه‌ی قیمت‌های تارگت/حد ضرر ثبت‌شده محاسبه شده‌اند، نه قیمت واقعی هر "
                "Fill روی صرافی؛ به‌خاطر Slippage ممکن است با سود/زیان واقعی کمی تفاوت داشته باشد.",
            ]
            return "\n".join(lines)[:4000]
        if cmd=="/pause":
