...
def recover_orphan_positions(nx: NobitexClient, gh: GithubClient, state: Dict[str, Any], only_known: bool = False) -> str:
    ...
    only_known=True (automatic mode) only adopts positions that appear in this
    executor's own history; the /recover command adopts any matching one.
    ...
    for p in active:
        ...
        orphans.append(p)
    if not orphans:
        return "" if only_known else "✅ هیچ پوزیشن بی‌صاحبی روی نوبیتکس نیست؛ همه‌ی پوزیشن‌های باز در state هستند."
    ...
    for p in orphans:
        ...
        if not cands:
            lines.append(f"❓ #{pid} {symbol} {side}: سیگنال مطابق در کانال پیدا نشد؛ دست نخورد (اگر دستی باز کرده‌اید طبیعی است).")
            continue
...
def auto_recover_orphans(nx: NobitexClient, gh: GithubClient, state: Dict[str, Any]) -> None:
    ...
    try:
        msg = recover_orphan_positions(nx, gh, state, only_known=True)
    except Exception as e:
...
def handle_bot_command(command: str, state: Dict[str, Any], nx: NobitexClient, gh: GithubClient) -> str:
    ...
    try:
        ...
        if cmd in ("/reconcile", "/sync"):
            ...
            _rec = recover_orphan_positions(nx, gh, state, only_known=True)
            if _rec:
                notify_admin(_rec)
            return "🔄 Reconciliation اجرا شد؛ گزارش کامل آن در تلگرام ارسال شد."
        if cmd == "/recover":
            return recover_orphan_positions(nx, gh, state, only_known=False)
