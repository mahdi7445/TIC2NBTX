...
# Minimum time between automatic retries of a still-unconfirmed emergency
# flatten (see resolve_needs_flatten). Keeps a persistently failing close from
# hammering the Nobitex API every single main-loop tick.
NEEDS_FLATTEN_RETRY_SECONDS = max(5, int(os.environ.get("NEEDS_FLATTEN_RETRY_SECONDS", "20")))
...
def _ack_button(key: str) -> list:
    """Standard 'reviewed, stop tracking automatically' button for a repeat
    alert about a specific trade key."""
    return [
        {"text": "✅ بررسی شد", "callback_data": f"ack:{key}"},
        {"text": "📈 معاملات باز", "callback_data": "cmd:/positions"},
    ]
...
class SignalFinalized(Exception):
    ...
    could not be protected and was deliberately emergency-flattened.
...
def resolve_leverage(nx: NobitexClient, src: str, dst: str, control: dict) -> Decimal:
    """Leverage to use for this specific market: the smaller of the admin's
    configured preference (control.json "leverage", changed via /leverage)
    and whatever Nobitex itself reports as the real maximum for this exact
    market. Most markets only support up to 5x, while a few (e.g. BTC, ETH at
    the time of writing) support up to 10x; reading the true ceiling from the
    exchange on every trade - instead of a fixed global number or a hardcoded
    coin list - means those markets automatically and permanently trade at
    their own real maximum as long as the admin preference is at least that
    high, with no per-symbol list to keep updated if Nobitex ever changes it.
    """
    ...
    if market_max is None or market_max <= 0:
        return preferred
    return max(Decimal("1"), min(preferred, market_max))
...
def _emergency_flatten(nx: NobitexClient, state: Dict[str, Any], key: str, trade: dict, reason: str,
                       quiet: bool = False) -> None:
    """Fail-safe: never leave a newly opened position intentionally unprotected.

    Always returns normally - it never raises. Callers that already committed
    to a terminal action (open-then-protect-or-flatten) raise SignalFinalized
    themselves; letting this function raise too was what previously let a
    single bad signal turn into an infinite open/re-flatten loop (poll_once
    would see the exception, refuse to mark the signal processed, and retry
    the whole thing from scratch). If the position can't be confirmed flat
    after retrying here, it is registered in needs_flatten so
    resolve_needs_flatten() keeps retrying safely on every main-loop
    iteration - that path only ever re-checks live status and re-attempts a
    market close, it never rebuilds protection orders or reopens a position.
    """
    if not quiet:
        log.error("EMERGENCY FLATTEN %s: %s", key, reason)
        notify_admin(f"🚨 FAIL-SAFE {key}: protection incomplete; attempting emergency market close. {reason}", key=key)
    else:
        log.error("EMERGENCY FLATTEN retry %s: %s", key, reason)
    ...
    for attempt in range(EMERGENCY_CLOSE_RETRIES):
        try:
            ...
            if not status or str(status.get("status", "")).lower() != "open":
                ...
                save_state(state)
                notify_admin(f"🛡️ {key}: emergency close confirmed; no unprotected position remains.", key=key)
                return
    ...
    # Could not confirm flat here. Keep the trade in open_trades (never
    # falsely report success) and hand it to the dedicated retry sweep.
    rec = state.setdefault("needs_flatten", {}).setdefault(key, {"first_seen": time.time(), "alerts_sent": 0})
    rec["reason"] = reason
    rec["position_id"] = trade.get("position_id")
    rec["last_attempt"] = time.time()
    save_state(state)
    if not quiet:
        notify_admin(
            f"🚨 CRITICAL {key}: emergency close could not be confirmed. MANUAL NOBITEX CHECK REQUIRED؛ "
            f"Executor به‌صورت خودکار و ایمن (بدون بازکردن معامله‌ی جدید) به تلاش برای بستن آن ادامه می‌دهد.",
...
def _finalize_opened_position(nx: NobitexClient, state: Dict[str, Any], key: str, sig: ParsedSignal,
    ...
    sizing checks and the exact same emergency-flatten fail-safe apply either
    ...
    try:
        ...
        notify_admin(
            f"🚨 {key}: پردازش سیگنال به دلیل خطای محافظت متوقف و پوزیشن بسته شد: {protection_error}{hint}",
            key=key, buttons=hint_buttons,
        )
...
def resolve_pending_protection(nx: NobitexClient, state: Dict[str, Any]) -> None:
    ...
    for key, rec in list(pending.items()):
        if key in state.get("open_trades", {}):
            ...
            continue
        try:
            position = nx.find_open_position_by_market(rec["src"], rec["dst"], rec["side"])
            if not position:
            ...
            # emergency-flattened - _finalize_opened_position/_emergency_flatten
...
def resolve_needs_flatten(nx: NobitexClient, state: Dict[str, Any]) -> None:
    """Keep retrying, on every main-loop iteration, to confirm a position is
    flat for trades where the initial emergency-flatten could not confirm
    closure (e.g. the "ExceedLiability"/"SmallOrder" cascade observed live on
    SOL_1H). This only ever re-checks live exchange status and re-attempts a
    market close for whatever is actually still open - it never rebuilds
    protection orders and never reopens a position, so unlike reprocessing
    the original channel signal, it is always safe to keep retrying.
    """
    needs = state.get("needs_flatten", {})
    if not needs:
        return
    for key, rec in list(needs.items()):
        if time.time() - float(rec.get("last_attempt", 0)) < NEEDS_FLATTEN_RETRY_SECONDS:
            continue
        trade = state.get("open_trades", {}).get(key) or {
            "position_id": rec.get("position_id"), "targets": {}, "runner": {},
        }
        if not trade.get("position_id"):
            needs.pop(key, None)
            continue
        try:
            _emergency_flatten(nx, state, key, trade, rec.get("reason", "retry"), quiet=True)
        except Exception:
        ...
        if key in needs:
            ...
            # only the first critical alert and then going quiet.
            if age > (sent + 1) * 300:
                notify_admin(
                    f"🚨 هنوز بسته‌شدن {key} تأیید نشده ({age/60:.0f} دقیقه از اولین تلاش)؛ "
                    f"لطفاً حساب Nobitex را فوراً به‌صورت دستی بررسی کنید.",
                    key=key, buttons=_ack_button(key),
                )
                needs[key]["alerts_sent"] = sent + 1
    state["needs_flatten"] = needs
    save_state(state)
...
def handle_signal(nx: NobitexClient, state: Dict[str, Any], sig: ParsedSignal, gh: GithubClient) -> None:
    ...
    if existing_trade:
        ...
        if not fully_protected:
            try:
                ...
                notify_admin(f"🚨 {key}: protection resume failed: {e}", key=key)
            ...
            try:
                ...
                notify_admin(f"🚨 {key}: existing protection verification failed: {e}", key=key)
    ...
    try:
        ...
        leverage = resolve_leverage(nx, src, dst, control)
        ...
        if position is None:
            ...
            # placing the order. If the position can't be located right after
            # the order is accepted (see _get_new_position_after_open), this
            # record lets resolve_pending_protection() keep searching on every
            # main-loop iteration until it is found and fully protected -
            # instead of the executor giving up and leaving a naked position,
            # which is exactly what happened live for BTC_5M and SOL_15M
            # before this fix.
...
def handle_event(nx: NobitexClient, state: Dict[str, Any], ev: ParsedEvent) -> None:
    ...
    if trade is None:
        notify_admin(f"⚠️ رویداد {ev.kind} برای {key} رسید ولی معامله‌ای در state باز نیست.", key=key)
        return
    ...
    if ev.kind == "target_hit":
        ...
        try:
            ...
            # Same fail-safe doctrine as everywhere else: a position that
            # cannot be confirmed fully protected right now must not be left
            # running unprotected between poll cycles.
            _emergency_flatten(nx, state, key, trade, f"post-target-hit protection rebuild failed: {rebuild_error}")
            save_state(state)
...
def reconcile_on_startup(nx: NobitexClient, state: Dict[str, Any]) -> None:
    ...
    pending = state.get("pending_protection", {})
    needs_flatten = state.get("needs_flatten", {})
    lines = [f"🔄 Startup reconciliation: state={len(state['open_trades'])}, Nobitex active={len(active)}, "
             f"pending_protection={len(pending)}, needs_flatten={len(needs_flatten)}"]
    if pending:
        lines.append(f"⏳ {len(pending)} pending-protection entry(ies) will be retried immediately: {', '.join(pending.keys())}")
    if needs_flatten:
        lines.append(f"🚨 {len(needs_flatten)} unconfirmed emergency-close entry(ies) will be retried immediately: {', '.join(needs_flatten.keys())}")
    for key, trade in list(state["open_trades"].items()):
...
def poll_once(nx: NobitexClient, gh: GithubClient, state: Dict[str, Any]) -> None:
    ...
    for line in lines:
        ...
        try:
            ...
            # (protected, or deliberately emergency-flattened because it
    ...
    try:
        ...
        if cmd in ("/status","/config"):
            ...
            return (f"📊 وضعیت\nExecutor: 🟢 online\nورود جدید: {'🟢 فعال' if c['enabled'] else '🔴 متوقف'}\n"
                    ...
                    f"Leverage preference: {c.get('leverage', DEFAULT_LEVERAGE)}x (per-market auto-capped)\n"
                    ...
                    f"🚨 Unconfirmed emergency closes: {len(state.get('needs_flatten',{}))}\nMargin USDT: {bal}")
        if cmd=="/balance":
        ...
        if cmd=="/leverage" and len(parts)>1:
            ...
            return (f"✅ اهرم ترجیحی به {v}x تغییر کرد.\n"
                    f"توجه: این یک سقف است — برای هر بازار، کمترین مقدار بین همین عدد و حداکثر مجاز "
                    f"همان بازار در نوبیتکس استفاده می‌شود (مثلاً بیت‌کوین/اتریوم معمولاً تا 10x مجازند و "
                    f"بقیه معمولاً تا 5x؛ این به‌صورت خودکار و بدون نیاز به تنظیم جدا برای هر ارز اعمال می‌شود).")
        if cmd=="/ack" and len(parts)>1:
            ...
            if k in state.get("needs_flatten", {}):
                state["needs_flatten"].pop(k, None); removed.append("needs_flatten")
            if removed:
                save_state(state)
            ...
            return f"ℹ️ موردی با کلید {k} در صف پیگیری خودکار (pending_protection/needs_flatten) پیدا نشد."
        if cmd=="/test_api":
...
def main() -> int:
    ...
    notify_admin(
        ...
        f"Leverage preference={_startup_control.get('leverage', DEFAULT_LEVERAGE)}x (per-market auto-capped) · "
