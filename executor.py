...
# ---------------------------------------------------------------------- #
# تنظیمات از متغیرهای محیطی (در GitHub Actions از Secrets پر می‌شوند)
# ---------------------------------------------------------------------- #
...
def main() -> None:
    ...
    log.info("شروع اجرا - base_url=%s leverage=%s risk_margin=%s USDT", NOBITEX_BASE_URL, LEVERAGE, RISK_MARGIN_USDT)
    ...
    while time.time() - start < LOOP_MAX_SECONDS:
        try:
            ...
            log.exception("خطا در حلقه‌ی اصلی")
            notify_admin(f"🚨 خطای حلقه‌ی اصلی: {e}")
        time.sleep(POLL_INTERVAL_SECONDS)

    log.info("پایان این اجرا (سقف زمانی رسید) - GitHub Actions اجرای بعدی را راه می‌اندازد.")
