...
_telegram: Optional[TelegramController] = None
...
def main() -> int:
    ...
    # Direct Telegram control plane: no long-running GitHub Actions bridge is required.
    global _telegram
    if TELEGRAM_BOT_TOKEN and TELEGRAM_CHANNEL_ID and TELEGRAM_ADMIN_CHAT_ID:
        _telegram = TelegramController(TELEGRAM_BOT_TOKEN, TELEGRAM_CHANNEL_ID, TELEGRAM_ADMIN_CHAT_ID, gh, handle_bot_command)
        _telegram.start()
    else:
        log.warning("Telegram control plane disabled: TELEGRAM_BOT_TOKEN/CHANNEL_ID/ADMIN_CHAT_ID missing")
