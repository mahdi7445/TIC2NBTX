...
def _finalize_opened_position(nx: NobitexClient, state: Dict[str, Any], key: str, sig: ParsedSignal,
    ...
    if live_liability <= 0:
        notify_admin(f"⚠️ {key}: پوزیشن پیدا شد اما liability آن صفر بود (احتمالاً از قبل بسته شده)؛ اقدامی انجام نشد.")
    ...
    try:
        ...
        notify_admin(f"🚨 {key}: پردازش سیگنال به دلیل خطای محافظت متوقف و پوزیشن بسته شد: {protection_error}{hint}")
...
def resolve_pending_protection(nx: NobitexClient, state: Dict[str, Any]) -> None:
    ...
    for key, rec in list(pending.items()):
        ...
        try:
            ...
            notify_admin(f"✅ {key}: positionId #{position.get('id')} پیدا شد؛ در حال ثبت SL/TP...")
            ...
            notify_admin(f"🚨 {key}: تلاش برای پیدا کردن/محافظت پوزیشن معلق شکست خورد: {e}")
