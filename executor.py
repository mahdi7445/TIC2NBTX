...
NEEDS_PROTECTION_RETRY_SECONDS = max(5, int(os.environ.get("NEEDS_PROTECTION_RETRY_SECONDS", "20")))
...
def _record_trade_history(state: Dict[str, Any], key: str, trade: dict, reason: str, note: str = "") -> None:
    """Append a closed trade to the persistent history log (state["trade_history"]).
    Pure record-keeping for the /history command - never reads back into, or
    affects, any trading decision, so it cannot introduce a behavior change.
    """
...
def resolve_leverage(nx: NobitexClient, src: str, dst: str, control: dict) -> tuple[Decimal, Decimal, Optional[Decimal]]:
    ...
    caller can show the admin exactly why a given value was chosen - this is
    the only way to tell, from the bot's own messages, whether a leverage
    that looks wrong is actually a stale-deployment issue (the preference
    itself never reaches this function) versus Nobitex reporting a lower
    market max than expected.
    """
    ...
    return max(Decimal("1"), min(preferred, market_max)), preferred, market_max
...
def _verify_trade_protection(nx: NobitexClient, trade: dict) -> None:
    ...
    for n in range(1, 5):
        ...
        if t.get("hit"):
            ...
            # paired SL leg as part of the OCO, so there is nothing live left
            # to verify here. Treating that as a failure was a real bug: it
            # made a routine /protection check (or a startup reconciliation)
            # misreport a perfectly healthy, partially-progressed trade as
            # broken.
            continue
...
def attempt_protection_repair(nx: NobitexClient, state: Dict[str, Any], key: str) -> str:
    """Try to complete this trade's protection using the exact same rebuild
    used after a normal target hit (cancel not-yet-hit targets/runner, then
    recreate them against the current live liability and current stop).
    Never closes the position. Safe to call repeatedly/automatically: on
    success it clears the needs_protection entry; on failure it re-flags
    (quietly - the periodic alert cadence handles admin notification) and
    the position is left exactly as it already was.
    """
    ...
    if not status or str(status.get("status", "")).lower() != "open":
        state.get("needs_protection", {}).pop(key, None)
        _record_trade_history(state, key, trade, "closed_externally", "detected during protection repair")
        state.get("open_trades", {}).pop(key, None)
        save_state(state)
        return f"ℹ️ {key}: پوزیشن دیگر روی نوبیتکس باز نیست (احتمالاً به حد سود/ضرر خودش رسیده)؛ از صف پیگیری خارج شد."
    try:
        _rebuild_protection(nx, trade, D(trade.get("stop_price", trade.get("original_stop"))))
        save_state(state)
        state.get("needs_protection", {}).pop(key, None)
        save_state(state)
        return f"✅ {key}: محافظت با موفقیت بازسازی و کامل شد."
    except Exception as e:
...
def handle_signal(nx: NobitexClient, state: Dict[str, Any], sig: ParsedSignal, gh: GithubClient) -> None:
    ...
    try:
        ...
        leverage, leverage_pref, leverage_market_max = resolve_leverage(nx, src, dst, control)
        ...
        leverage_detail = (
            f"Leverage={leverage}x (ترجیح شما: {leverage_pref}x · حداکثر بازار طبق نوبیتکس: "
            f"{leverage_market_max if leverage_market_max is not None else 'نامشخص'}x)"
        )
        ...
        notify_admin(
            ...
            f"Risk cap=${risk_cap} · Effective risk≈${effective_risk:.4f} · {leverage_detail}\n"
...
def _close_target_if_needed(nx: NobitexClient, trade: dict, key: str, target_num: int) -> None:
    ...
    notify_admin(
        ...
        + _stop_vs_liquidation_note(trade, _position_status(nx, trade)),
        key=key,
    )
...
def _close_remaining_at_price(nx: NobitexClient, state: Dict[str, Any], key: str, reason: str, price: Decimal) -> None:
    ...
    status = _position_status(nx, trade)
    if not status or str(status.get("status", "")).lower() != "open":
        _record_trade_history(state, key, trade, reason, "already closed before this event was processed")
    ...
    try:
        ...
        _record_trade_history(state, key, trade, reason, f"closed at strategy price {price}")
        state["open_trades"].pop(key, None)
        save_state(state)
...
def _close_remaining_market(nx: NobitexClient, state: Dict[str, Any], key: str, reason: str) -> None:
    ...
    status = _position_status(nx, trade)
    if not status or str(status.get("status", "")).lower() != "open":
        _record_trade_history(state, key, trade, reason, "already closed before this command/event was processed")
        state["open_trades"].pop(key, None)
        save_state(state)
    ...
    _record_trade_history(state, key, trade, reason, "closed via market order")
    state["open_trades"].pop(key, None)
    save_state(state)
...
def update_runner_trailing(nx: NobitexClient, state: Dict[str, Any]) -> None:
    ...
    for key, trade in list(state["open_trades"].items()):
        ...
        try:
            ...
            status = _position_status(nx, trade)
            if not status or str(status.get("status", "")).lower() != "open":
                _record_trade_history(state, key, trade, "closed_externally", "detected during runner trailing update")
                state["open_trades"].pop(key, None)
                continue
            liability = D(status.get("liability", "0"))
            if liability <= 0:
    ...
    try:
        ...
        if cmd=="/positions":
            ...
            for key, t in sorted(trades.items()):
                ...
                lines.append(
                    f"\n🔹 {key} · {t.get('side','?')}\n"
                    f"Entry: {t.get('entry_actual','?')} · Stop فعلی: {t.get('stop_price','?')}\n"
                    f"تارگت‌ها: {' '.join(marks)} · Runner: {runner_state}\n"
                    f"Liability: {live.get('liability', t.get('initial_amount','?'))} · "
                    f"PNL: {live.get('unrealizedPNL', '؟')} · Position ID: {pid}"
        ...
        if cmd=="/history":
            ...
            for h in recent:
                ...
                if opened and closed:
                    ...
                    dur = f" · مدت: {mins//60}h{mins%60}m" if mins >= 60 else f" · مدت: {mins}m"
                hit = ", ".join(h.get("targets_hit") or []) or "هیچ‌کدام"
                pct = h.get("closed_pct", "0")
                lines.append(
                    f"\n🔸 {h.get('key')} · {h.get('side','?')} · نتیجه: {h.get('reason','?')}\n"
                    f"Entry: {h.get('entry_actual','?')} → Stop نهایی: {h.get('final_stop','?')}\n"
                    f"تارگت‌های رسیده: {hit} (٪{pct} حجم بسته‌شده) · Runner trailing: {'بله' if h.get('trailing_active') else 'خیر'}\n"
                    f"باز شد: {opened_s} · بسته شد: {closed_s}{dur}"
