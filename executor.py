...
def _note_network_trouble(context: str, err: Exception) -> None:
    now = time.time()
    first = _network_issue["since"] == 0.0
    if first:
        _network_issue["since"] = now
    backoff = min(NETWORK_ALERT_MAX_SECONDS, NETWORK_ALERT_BASE_SECONDS * (2 ** _network_issue["count"]))
    if first or now - _network_issue["last_alert"] >= backoff:
        down_for = now - _network_issue["since"]
        next_backoff = min(NETWORK_ALERT_MAX_SECONDS, NETWORK_ALERT_BASE_SECONDS * (2 ** (_network_issue["count"] + 1)))
        notify_admin(
            f"⚠️ اتصال Executor به نوبیتکس/اینترنت قطع است (از {down_for/60:.0f} دقیقه پیش، در {context}).\n"
            f"آخرین خطا: {err}\n\n"
            f"این معمولاً یک مشکل DNS/VPN/اینترنت روی همین کامپیوتر ویندوز است، نه باگ نرم‌افزار "
            f"(پیام «Failed to resolve api.nobitex.ir» یعنی خود ویندوز نتوانسته آدرس را پیدا کند). "
            f"لطفاً VPN/DNS/اتصال اینترنت را بررسی کنید. Executor خودش هر چند ثانیه دوباره تلاش می‌کند و "
            f"وقتی وصل شد یک پیام «اتصال برقرار شد» می‌فرستد؛ تا آن زمان دیگر این هشدار تکرار نمی‌شود "
            f"(پیام بعدی حداکثر تا {next_backoff/60:.0f} دقیقه‌ی دیگر، اگر هنوز قطع باشد).",
        )
        _network_issue["last_alert"] = now
        _network_issue["count"] += 1


def _note_network_ok() -> None:
    if _network_issue["since"] != 0.0:
        down_for = time.time() - _network_issue["since"]
        notify_admin(f"✅ اتصال Executor به نوبیتکس دوباره برقرار شد (بعد از حدود {down_for/60:.0f} دقیقه قطعی).")
